...
photos: >-
  https://gitee.com/shmilyqjj/BlogImages/raw/master/cdn_sources/Kyuubi/Kyuubi-cover.jpg
abbrlink: ee1c2df4
date: 2022-04-29 16:39:12
---
# Kyuubi原理与替代SparkThriftServer实践-基于CDH6
![alt](https://gitee.com/shmilyqjj/BlogImages/raw/master/cdn_sources/Kyuubi/Kyuubi-00.png)
...
| 系统架构 | ![alt](https://gitee.com/shmilyqjj/BlogImages/raw/master/cdn_sources/Kyuubi/Kyuubi-01.png) | ![alt](https://gitee.com/shmilyqjj/BlogImages/raw/master/cdn_sources/Kyuubi/Kyuubi-08.png) |


## Kyuubi原理
### Kyuubi架构图
![alt](https://gitee.com/shmilyqjj/BlogImages/raw/master/cdn_sources/Kyuubi/Kyuubi-01.png)
...
![alt](https://gitee.com/shmilyqjj/BlogImages/raw/master/cdn_sources/Kyuubi/Kyuubi-02.png)
Kyuubi支持不同共享级别的引擎共享。如果设置了USER级别的share.level，同一用户与Kyuubi建立的多个连接会复用同一个Engine，实现用户级别的资源隔离。

### Kyuubi资源隔离共享级别
| 共享级别 | 参数 | 图解 | 说明 |
|----|----|----|----|
| CONNECTION | kyuubi.engine.share.level=CONNECTION | ![alt](https://gitee.com/shmilyqjj/BlogImages/raw/master/cdn_sources/Kyuubi/Kyuubi-09.png) | 每个连接都创建一个独立的Engine，连接创建即申请Engine，连接关闭即释放Engine |
| USER | kyuubi.engine.share.level=USER | ![alt](https://gitee.com/shmilyqjj/BlogImages/raw/master/cdn_sources/Kyuubi/Kyuubi-10.png) | 同一用户的多个连接共享一个Engine，一个用户对应一个Engine，用户连接关闭后不会立刻释放Engine，在无操作达到TTL后释放Engine |
| GROUP | kyuubi.engine.share.level=GROUP | ![alt](https://gitee.com/shmilyqjj/BlogImages/raw/master/cdn_sources/Kyuubi/Kyuubi-11.png) | 属于相同组的所有用户创建的所有连接共享同一个Engine，以组名作为启动Engine的用户名，数据权限按组进行管理，如果组名不存在，共享级别降级为USER，用户组遵循[Hadoop Groups Mapping](https://hadoop.apache.org/docs/current/hadoop-project-dist/hadoop-common/GroupsMapping.html)，可以通过配置把不同用户映射到一个组。相比USER级别给每个用户都创建引擎，GROUP级别可以减少引擎实例数，节约资源，但引擎是共享的，同组所有用户都复用这个引擎，访问权限控制若要做到细粒度，则需要结合[Apache Ranger](https://ranger.apache.org/)，资源控制的细粒度需要结合[SparkFairScheduler](https://spark.apache.org/docs/latest/job-scheduling.html#fair-scheduler-pools) | 
| SERVER | kyuubi.engine.share.level=SERVER | ![alt](https://gitee.com/shmilyqjj/BlogImages/raw/master/cdn_sources/Kyuubi/Kyuubi-12.png) | 每个KyuubiServer中的连接共用一个Engine，类似原生ThriftServer的高可用版本 |
...
```
可以看到单个用户启动了两个Engine
![alt](https://gitee.com/shmilyqjj/BlogImages/raw/master/cdn_sources/Kyuubi/Kyuubi-13.png)
...
![alt](https://gitee.com/shmilyqjj/BlogImages/raw/master/cdn_sources/Kyuubi/Kyuubi-14.png)
...
![alt](https://gitee.com/shmilyqjj/BlogImages/raw/master/cdn_sources/Kyuubi/Kyuubi-06.png)
...
![alt](https://gitee.com/shmilyqjj/BlogImages/raw/master/cdn_sources/Kyuubi/Kyuubi-03.png)
...
![alt](https://gitee.com/shmilyqjj/BlogImages/raw/master/cdn_sources/Kyuubi/Kyuubi-05.png)

3. 使用LDAP登录的用户无HDFS上表数据的访问权限
![alt](https://gitee.com/shmilyqjj/BlogImages/raw/master/cdn_sources/Kyuubi/Kyuubi-04.png)
分析：需要确保当前用户的权限或者ACL权限是READ_EXECUTE
![alt](https://gitee.com/shmilyqjj/BlogImages/raw/master/cdn_sources/Kyuubi/Kyuubi-07.png)
