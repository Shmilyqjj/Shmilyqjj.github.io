...
photos: >-
  http://imgs.shmily-qjj.top/BlogImages/DataMasking/DataMasking-cover.jpg
abbrlink: f5da73a2
...
## 大数据脱敏技术方案
先上一个总体的脑图：[数据脱敏脑图XMIND](http://imgs.shmily-qjj.top/BlogImages/DataMasking/数据脱敏脑图.xmind)
### 批量数据脱敏
1. 先将数据同步到数据仓库的一个中间表，然后通过一些自定义的脱敏函数udf进行数据脱敏，然后将未脱敏的中间表删除或通过权限管控起来即可。对于存量数据也是经过UDF处理后得到脱敏数据。
![alt DataMasking-1](http://imgs.shmily-qjj.top/BlogImages/DataMasking/DataMasking-1.JPG)
手机号脱敏UDF示例:
```java
...
![alt DataMasking-2](http://imgs.shmily-qjj.top/BlogImages/DataMasking/DataMasking-2.JPG)

### 数据安全审计
数据安全审计，目前业界常用的就是ELK(Es+LogStash+Kibana)，下面是用于大数据审计的流程
![alt DataMasking-3](http://imgs.shmily-qjj.top/BlogImages/DataMasking/DataMasking-3.JPG)
